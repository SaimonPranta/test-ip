...
const index = ({
    ...
    const openDiv = async (name) => {
        ...
        if (activeField.next === name) {
            ...
            if (faceIdCard[name].click) return
            setFaceIdCard(prev => {
                return {
                    ...prev,
                    [name]: {
    ...
    return (
        <>
            <div
                ...
                className={`verification-inner-container ${warningDetector(name)} ${getDisable(name)}`}>
                {!faceIdCard[name].click && <FaCamera
                    className={'show-camera-icon'}>
                </FaCamera>}
                <div className='show-vd-info-parent-div'>
                    {faceIdCard[name].click && <div className='show-video-info-div'>
                        {
                            !faceIdCard[name].cameraOn
                                ? <div className='infoDiv'>
                                    <div className='info'>{information.map((i, index) => <p key={index}><span className='index-number'>({index + 1})</span>{i}</p>)}</div>
